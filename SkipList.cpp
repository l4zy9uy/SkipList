...
#include "SkipList.h"
template class SkipList<int, std::less<int>>;
...
bool SkipList<T, Compare>::erase(const T &value) {
    std::vector<Node *> update(maxLevel_, nullptr);
    Node *x = head_;
    // 1) Find nodes just before the target
    for (int i = currentLevel_ - 1; i >= 0; --i) {
        while (x->next[i] && compare_(x->next[i]->value, value)) {
            x = x->next[i];
        }
...
template<typename T, typename Compare>
typename SkipList<T, Compare>::Node *SkipList<T, Compare>::begin() const {
    return head_->next[0];
}

template<typename T, typename Compare>
typename SkipList<T, Compare>::Node *SkipList<T, Compare>::end() const {
    return nullptr;
}

template<typename T, typename Compare>
typename SkipList<T, Compare>::Node *SkipList<T, Compare>::lower_bound(const T &key) {
    Node *x = head_;
    for (int i = currentLevel_ - 1; i >= 0; --i) {
...
template<typename T, typename Compare>
typename SkipList<T, Compare>::Node *SkipList<T, Compare>::upper_bound(const T &key) {
    Node *x = head_;
    for (int i = currentLevel_ - 1; i >= 0; --i) {
